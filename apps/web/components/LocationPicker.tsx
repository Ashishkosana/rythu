    ...
    <div className="mb-3">
      ...
      >
        <span className="flex items-center gap-2">
          ...
          <span className="font-semibold text-slate-800">{currentPlace}</span>
        </span>
      ...
      {open && (
        <div className="mt-2 space-y-3 rounded-xl border border-slate-200 bg-white p-3">
          ...
          <div>
            <p className="mb-1 text-xs font-medium text-slate-500">భూపాలపల్లి జిల్లా · Quick pick</p>
          ...
          <div>
            <input
              ...
              className="w-full rounded-xl border border-slate-300 px-4 py-2.5 outline-none focus:border-green-600"
            />
            {searching && <p className="mt-1 text-xs text-slate-400">వెతుకుతోంది…</p>}
            {results.length > 0 && (
              <ul className="mt-1 max-h-56 overflow-auto rounded-xl border border-slate-200">
                {results.map((r) => (
                  <li key={r.id}>
                    <button
            ...
            {!searching && query.trim().length >= 2 && results.length === 0 && (
              <p className="mt-1 text-xs text-slate-400">ఫలితాలు లేవు · No results</p>
            )}
          </div>
        </div>
      )}
